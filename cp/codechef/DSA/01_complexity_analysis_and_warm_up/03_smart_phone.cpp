...
Note:
The answer might not fit in a variable of type int. We recommend 
that you use variables of type long long to read the input and compute the answer. 
...
int main()
{
    ...
    {
        ll thissale = budgets[i]*(budgets.size()-i);
        if (thissale > sale)
            sale = thissale;
    }
